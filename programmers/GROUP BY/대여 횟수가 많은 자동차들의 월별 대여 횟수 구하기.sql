...
SELECT MONTH(START_DATE) AS MONTH, CAR_ID, COUNT(HISTORY_ID) AS RECORDS # 총 대여 횟수(컬럼명: RECORDS)
...
AND DATE_FORMAT(START_DATE, '%Y%m') BETWEEN '202208' AND '202210' # 해당 기간 동안의
GROUP BY CAR_ID, MONTH(START_DATE) # 월별 자동차 ID 별
ORDER BY MONTH(START_DATE), CAR_ID DESC # 월을 기준으로 오름차순 정렬하고, 월이 같다면 자동차 ID를 기준으로 내림차순 정렬
